...
Первый этап курса готовит не приложение, а управляемую среду для агентной разработки. После урока в рабочем проекте должны быть правила, агенты, навыки и конфигурация opencode, которые можно сразу использовать на следующих этапах курса.
...
Папка `ASSETS` содержит готовый набор файлов, который можно целиком скопировать в новый проект и сразу открыть этот проект в opencode.
...
Создайте пустую папку будущего проекта или откройте уже существующий проект, в котором дальше будет разрабатываться приложение курса.
...
На этом этапе не создавайте `frontend`, `backend`, базу данных, Docker-файлы и тесты. Первый урок отвечает только за агентную среду.
...
```powershell
Copy-Item -Recurse -Force .\Course\01-agent-environment\ASSETS\* .\task-manager-ai\
```
...
`AGENTS.md` - это проектные правила для opencode. Файл должен лежать в корне проекта и описывать:
...
opencode умеет работать с файлами, искать по проекту и запускать команды через терминал. Но системные команды должны быть установлены отдельно.
...
```text
@architect подготовь план вопросов для технического задания приложения "Менеджер задач с ИИ". Не создавай код и не создавай структуру приложения.
```

Проверка навыка ТЗ:

```text
Подготовь структуру технического задания для приложения "Менеджер задач с ИИ". Если нужен навык product-spec, используй его. Код пока не создавай.
```
...
Первый урок завершен, если можно ответить "да" на все вопросы:
