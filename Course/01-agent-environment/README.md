...
- подключает `AGENTS.md` и `SKILLS.md` как проектные инструкции через `instructions`;
...
Если совсем просто, agents - это заранее описанные роли для ИИ. Например, одному агенту мы говорим думать как архитектору, другому - как backend-инженеру, третьему - как QA. Это помогает не просить одного универсального помощника обо всем сразу, а направлять задачу к роли с понятной ответственностью.
...
```text
Прочитай AGENTS.md и SKILLS.md. Ничего не меняй. Кратко объясни:
...
5. какие внешние программы должны быть установлены отдельно.
```
